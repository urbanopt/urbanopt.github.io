...
nav_order: 2
---
# URBANopt-REopt Energy Resilience Planning
...
It models how resources such as solar, battery and generators can sustain critical loads during grid outages, considering technology reliability, configurarion and dispatch strategies for detailed resilience planning. [REopt ERP documentation](https://github.com/NREL/REopt-Analysis-Scripts/wiki/4.-API-endpoints#hoststableerp) goes over the workflow and commands for running the capability.
...
This folder contains the REopt assumption files, which include the required inputs and default values for running a REopt analysis. For ERP, two additional assumption files are included: `multiPV_assumptions_ERP.json` and `erp_assumptions.json`.

 The `multiPV_assumptions_ERP.json` defines an array of PV inputs used to optimize multiple PV systems simultaneously. More details on creating a REopt example project and the structure of these assumption files are available in the [REopt Post Processing Section](./reopt_post_processing.md).
 
 This file also contains the key input fields required to run an ERP analysis, namely:
...
More details on these inputs is provided [here](https://developer.nrel.gov/api/reopt/stable/help/?API_KEY=DEMO_KEY).

The `erp_assumptions.json` contains the following:

`max_outage_duration`. This is a required input to run the ERP capability. By default it is set to 24 hours. This can be updated by the user and provided as a flag when running the ERP analysis. If this file is not added an argument while running the ERP capability, a default of 24 hours is assumed.
...
2. **Run ERP on an existing system without outage-based sizing.**
    ...
    REopt sizes PV and storage purely based on economic objectives (e.g., cost savings, emissions), with no resilience requirement. When running the ERP post procesing,REopt uses the system sizes and dispatch profile generated previously, It then simulates outages starting at every hour of the year, using the actual battery state of charge at each hour. This is often useful if the user wants to first size for economics, then  see how resilient the system is without explicitly the sizing toward resilience.
...
```bash
uo create ----reopt-erp-scenario-file <path/to/SCENARIOFILE.json>
```

The ERP scenario file, assigns the `multiPV_assumptions_ERP.json` to the features in the URBANpot project. The instructions to create an URBANopt and REopt scenario file are also provided in the [Getting Started](../../getting_started/getting_started.md).

3. ### Run URBANopt project
 
The URBANopt project is run by adding the -f feature file and the REopt ERP Scenario file
...
More details about the results can be found [here](https://developer.nrel.gov/api/reopt/stable/erp/outputs?API_KEY=DEMO_KEY).